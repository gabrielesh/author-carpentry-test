...
title: Data Management Plan for the Study _Do Reputable Open Access Journals Require
  Open Data Sharing?_
author: Principal Investigator - Gabriele Hayden, `r params$institution`
date: "`r format(Sys.Date(), '%A, %B %d, %Y')`"
bibliography: 
- oajournals.bib
- packages.bib
# citer package for live? integration w/ Zotero. A bit wonky, not on CRAN
# note you can also manually add reference here in the YAML header
# csl: apa.csl
params:
  institution:
    choices:
    ...
    input: select
    label: 'Institution:'
    value: University of Oregon Libraries 
  fee:
    choices:
    - True
    - False
    ...
    value: True
# Default value required to be able to run individual code chunks without error (otherwise it will crash RStudio)
output: 
  html_document:
    toc: yes
    toc_depth: 1
    ...
    toc_float: yes
    theme: readable
#    css: custom.css refer to custom css file of that name to add custom css config
    code_folding: hide # creates show/hide button for code in doc, default hide. Only works in html code_folding: show defaults to showing code
  word_document:
    toc: yes
    toc_depth: 1
#    reference_docx: Base_2013_style.doc make custom styles in this doc as template
---
...
Gabriele Hayden, Principal Investigator, `r params$institution`

## Researcher ID

ORCID: https://orcid.org/0000-0003-4740-4187

## Date of First Version 
`r format(Sys.Date()-60, '%A, %B %d, %Y')`
(In reality this would be a set date, not programmed.)
...
a. All original data, code, or reports produced as part of this project are owned by `r params$institution` per its institutional intellectual property policy.
    * This is a sub-bullet for this item
...
A sample of the `doaj_seal.csv` data set is shown below. The complete data set is available in searchable and  broweseable format as [Annex 9.1](#annex-table) at the end of this document.   
...
2. 'Data Sharing Policy' (Factor, 6 levels)
    1. No mention 
    2. Implied
    3. Mentioned
...
3. 'Data Citation Policy' (Factor, 3 levels)
    1. No mention 
    2. Implied
...
guides(fill=guide_legend("License"))
```
...
[comment]: # (Not recommended to use w/ MS Word knitting, as it either errors out or prints the whole table)
<!--- this is another way to comment, but it may flow through as HTML source code --->
...
This field is auto-populated from an ORCID profile using @R-rorcid package.
...
res <- orcid_bio(orcid = "0000-0003-4740-4187")
bio <- res$'0000-0003-4740-4187'$'content'
